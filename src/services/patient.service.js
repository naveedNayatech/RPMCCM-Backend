...
const uploadConsent = async (body) => {
  ...
  let findPatient = await Consent.findOne({ assigned_patient_id: body.assigned_patient_id })
...
const getPatientCCMConsent = async (patientId) => {
  let consent = await Consent.findOne({ assigned_patient_id: patientId }).lean()
    .populate("assigned_patient_id")

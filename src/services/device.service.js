...
const forwardtelemetry = async (body) => {
  ...
  if (patient === null) {
    throw new ApiError(httpStatus.NOT_FOUND, 'Patient not found with this deviceID');
  }
    ...
    insertIntoDeviceData = await HealthData.create({
      ...
      dateAdded: moment(new Date()).tz("America/New_York").format("YYYY/MM/DD"),
      time:moment(new Date()).tz("America/New_York").format("hh:mm z" )
    })

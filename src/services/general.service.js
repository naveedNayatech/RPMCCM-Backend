...
const initialSetupReport = async (body) => {
    ...
    else if (body.month && body.hrId) {
        query = { "initialsetup": body.month, "assigned_hr_id": body.hrId }
    }
    else {
...
const minutereadingtotaldrhr = async (body) => {
    if ((body.drId || body.hrId) && body.month && body.year) {
        ...
        if (body.drId)
            patientList = await Patient.find(Patient.find({ $or: [ { patientType: 'RPM' }, { patientType: 'Both' } ], block: false }).lean());

        if (body.hrId)
            patientList = await Patient.find(Patient.find({ $or: [ { patientType: 'RPM' }, { patientType: 'Both' } ], assigned_hr_id: body.hrId, block: false }).lean());
    ...
    if ((body.drId || body.hrId) && body.month && body.year) {
        ...
        if (body.hrId)
            patientList = await Target.find({
                ...
                createdAt: {
                    '$gte': new Date(new Date(`${body.year}-${body.month}-01`).setHours(00, 00, 00)),
